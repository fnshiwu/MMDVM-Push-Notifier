#!/bin/bash
# MMDVM-Push-Notifier auto update script (v3.3.0-pistar4.3.7) | 核心全自动更新脚本
# Target platforms: Pi-Star 4.2.x - 4.3.7 / Debian 11-12 | 适用平台扩展
...
echo "Detected Pi-Star: $PISTAR_VER, Debian: $DEBIAN_VER"
...
# =========================
# Ensure sudoers rule exists (4.3.5+ compatible) | 确保 sudoers 规则存在（4.3.5+ 兼容）
# =========================
...
EOF
chmod 440 "$SUDO_D"
visudo -cf "$SUDO_D" >/dev/null 2>&1 || { echo "⚠️  Sudoers validation failed"; rm -f "$SUDO_D"; }
fi
