...
class HamInfoManager:
    def __init__(self, id_file):
        ...
        self._last_mtime = 0  # [S+] CSV mtime 跟踪
        self.geo_map = {
            # --- 原映射表，未改动 ---
    ...
    def get_info(self, callsign):
        try:
            mtime = os.path.getmtime(self.id_file)
            if mtime != self._last_mtime:
                self.get_info.cache_clear()  # [S+] CSV 更新即清缓存
...
class PushService:
    ...
    def _do_push_logic(cls, config, type_label, body_text, is_voice):
        if not cls._push_semaphore.acquire(blocking=False):  # [S+] 防线程池堆死
            return
        try:
            # --- 原推送逻辑，未改动 ---
...
class MMDVMMonitor:
    ...
    def process_line(self, line):
        ...
        curr_ts = time.time()
        delta = curr_ts - self.last_msg["ts"]
        if delta < 0:  # [S+] 系统时间回拨保护
            self.last_msg["ts"] = 0
    ...
    # === 其余方法保持原样 ===
    # run / get_sys_info / temp / quiet / main
